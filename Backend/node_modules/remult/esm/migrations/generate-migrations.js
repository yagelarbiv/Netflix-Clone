...
import fs from 'fs';
import { compareMigrationSnapshot, emptySnapshot, } from './compare-migration-snapshots.js';
import { updateMigrationsFile } from './update-migrations-ts-file.js';
import { DefaultMigrationBuilder, } from './migration-types.js';
...
export async function generateMigrations(options) {
    const migrationDir = options.migrationsFolder || path.join(process.cwd(), 'src/migrations');
    const snapshotFileName = options.snapshotFile || path.join(migrationDir, 'migrations-snapshot.json');
    const migrationsTSFilename = options.migrationsTSFile || path.join(migrationDir, 'migrations.ts');
    ...
    const code = {
        ...
        addComment: (comment) => steps.push(comment.indexOf('\n') >= 0 ? `/*${comment}*/` : `// ${comment}`),
    };
    let migrationBuilder = new DefaultMigrationBuilder(code);
    if (isOfType(dataProvider, 'provideMigrationBuilder')) {
        migrationBuilder = new DefaultMigrationBuilder(code, dataProvider.provideMigrationBuilder(code));
    }
    let reporter = (what) => console.log('[generateMigrations] ' + what);
