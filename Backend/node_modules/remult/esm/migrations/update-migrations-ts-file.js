...
export async function updateMigrationsFile(filePath, steps) {
    ...
    let maxStep = -1;
    function buildSteps() {
        return steps.map((step) => ts.factory.createPropertyAssignment(ts.factory.createIdentifier((++maxStep).toString()), ts.factory.createIdentifier(step)));
    }
                ...
                const visitor = (node) => {
                    if (ts.isVariableStatement(node) &&
                        ...
                        if (ts.isIdentifier(declaration.name) &&
                            ...
                            const updatedInitializer = ts.factory.updateObjectLiteralExpression(declaration.initializer, [...declaration.initializer.properties, ...steps]);
                            // Update the declaration with the new initializer
                            const updatedDeclaration = ts.factory.updateVariableDeclaration(declaration, declaration.name, undefined, declaration.type, // Preserve the original type annotation if present
                            updatedInitializer);
    ...
    if (!migrationsExportFound) {
        ...
        if (!importFound) {
            updatedSourceFile = ts.factory.updateSourceFile(updatedSourceFile, [
                ts.factory.createExpressionStatement(ts.factory.createIdentifier(`import type { ${importName} } from '${importPath}'`)),
                ...updatedSourceFile.statements,
            ]);
        }
        // Create a new export for migrations if it doesn't exist
        const newMigrationsExport = ts.factory.createVariableStatement([ts.factory.createModifier(ts.SyntaxKind.ExportKeyword)], ts.factory.createVariableDeclarationList([
