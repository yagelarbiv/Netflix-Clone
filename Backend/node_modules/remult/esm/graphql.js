...
export function remultGraphql(options) {
    ...
    function upsertTypes(key, kind = 'type', order = 0) {
    ...
    for (const meta of entities) {
        ...
        if (key) {
            const createResultPromise = (work) => {
                return async (arg1, req) => {
            ...
            const handleRequestWithDataApiContextBasedOnRepo = (repo, work) => {
            ...
            const handleRequestWithDataApiContext = (work) => {
                return createResultPromise(async (response, setResult, arg1, req) => {
            ...
            const handleMutationWithErrors = (work) => {
                return handleRequestWithDataApiContext(async (dApi, response, origSetResult, arg1, req) => {
            ...
            const queryArgsConnection = getQueryArgsConnection(key);
            ...
            root[getSingleEntityKey] = handleRequestWithDataApiContext(async (dApi, response, setResult, arg1, req) => {
                await dApi.get({
                    ...response,
                    success: (y) => {
                        const orig = { ...y };
            ...
            resolversQuery[getSingleEntityKey] = (origItem, args, req, gqlInfo) => root[getSingleEntityKey](args, req, gqlInfo);
            ...
            root[key] = handleRequestWithDataApiContext(async (dApi, response, setResult, arg1, meta) => {
                connectionImplementation(setResult, itemsKey, createResultPromise, dApi, currentType, arg1, meta, totalCountKey);
            });
            resolversQuery[key] = (origItem, args, req, gqlInfo) => {
                return root[key](args, req, gqlInfo);
            };
            // Mutation
            const root_mutation = upsertTypes('Mutation', 'type', -9);
            const checkCanExist = (rule) => rule !== false &&
                !(rule === undefined &&
            ...
            if (checkCanExist(meta.options.allowApiInsert)) {
                ...
                root[createResolverKey] = handleMutationWithErrors(async (dApi, response, setResult, arg1, req) => {
                    await dApi.post({
                        ...response,
                        created: (y) => {
                            const orig = { ...y };
                ...
                resolversMutation[createResolverKey] = (origItem, args, req, gqlInfo) => root[createResolverKey](args, req, gqlInfo);
            }
            if (checkCanExist(meta.options.allowApiUpdate)) {
                ...
                const updateInput = `Update${getMetaType(meta)}Input`;
                {
                    ...
                    resolversMutation[updateResolverKey] = (origItem, args, req, gqlInfo) => handleMutationWithErrors(async (dApi, response, setResult, arg1, req) => {
                        await dApi.put({
                            ...response,
                            success: (y) => {
                                const orig = { ...y };
                ...
                // update many
                {
                    const updateManyPayload = `UpdateMany${upperFirstChar(pluralEntityKey)}Payload`;
                    const updateManyResolverKey = `updateMany${upperFirstChar(pluralEntityKey)}`;
                    root_mutation.fields.push({
                    ...
                    resolversMutation[updateManyResolverKey] = (origItem, args, req, gqlInfo) => handleMutationWithErrors(async (dApi, response, setResult, arg1, req) => {
            ...
            if (checkCanExist(meta.options.allowApiDelete)) {
                // delete
                {
                    ...
                    const type = meta.idMetadata.field instanceof CompoundIdField
                        ? 'String'
                    ...
                    resolversMutation[deleteResolverKey] = (origItem, args, req) => handleMutationWithErrors(async (dApi, response, setResult, arg1, req) => {
                        await dApi.delete({
                            ...response,
                            deleted: () => {
                ...
                // delete
                {
                    const deleteManyPayload = `DeleteMany${upperFirstChar(pluralEntityKey)}Payload`;
                    const deleteManyResolverKey = `deleteMany${upperFirstChar(pluralEntityKey)}`;
                    root_mutation.fields.push({
                    ...
                    resolversMutation[deleteManyResolverKey] = (origItem, args, req) => handleMutationWithErrors(async (dApi, response, setResult, arg1, req) => {
                        await dApi.deleteMany({
                            ...response,
                            success: (deleteManyResult) => {
                                setResult(deleteManyResult);
                            },
                        }, { get: () => undefined }, translateWhereToRestBody(meta.fields, arg1));
                    })(args, req);
                }
            }
            const whereTypeFields = [];
            for (const f of meta.fields) {
                ...
                if (ri) {
                    {
                        ...
                        ref = entities.find((i) => i.entityType === refType);
                    }
                    if (!ref) {
                        throw new Error(`Entity "${ri.toEntity.name}" that is used by the relation "${f.key}" in "${meta.entityType.name}" was not found in the 'entities' array.`);
                    }
                    ...
                    switch (ri.type) {
                        ...
                        case 'toOne':
                            ...
                            currentType.query.resultProcessors.push((r, orig) => {
                                r[f.key] = async (args, req, gqlInfo) => {
                                    const remult = await options.getRemultFromRequest(req);
                                    ...
                                    if (returnNull || returnUndefined)
                                        return null;
                                    const result = await handleRequestWithDataApiContextBasedOnRepo(toRepo, async (dApi, response, setResult, arg1, meta) => {
                                        return await connectionImplementation(setResult, itemsKey, createResultPromise, dApi, upsertTypes(getMetaType(ref)), arg1, meta, totalCountKey);
                        ...
                        case 'toMany':
                            ...
                            currentType.query.resultProcessors.push((r, y) => {
                                r[f.key] = async (args, req, gqlInfo) => {
                                    ...
                                    let relRepo = myRepo.relations(item)[f.key];
                                    return handleRequestWithDataApiContextBasedOnRepo(relRepo, async (dApi, response, setResult, arg1, meta) => {
                                        return await connectionImplementation(setResult, itemsKey, createResultPromise, dApi, upsertTypes(getMetaType(ref)), arg1, meta, totalCountKey);
                                    })(args, req);
                                };
                            });
                    ...
                    currentType.query.resultProcessors.push((r) => {
                        ...
                        if (val === null || val === undefined)
                            return null;
                        r[f.key] = async (args, req, gqlInfo) => {
                            const queryResult = await (await root[refKey]({
                                ...args.where,
                                where: { id: { eq: val } },
                                options: { limit: 1 },
                            }, req, gqlInfo)).items();
                    ...
                    refT.query.resultProcessors.push((r) => {
                        const val = r.id;
                        r[keyOf] = async (args, req, gqlInfo) => {
                ...
                const includeInUpdateOrInsert = f.options.allowApiUpdate !== false && !notARealField;
    ...
    resolversQuery[nodeKey] = (origItem, args, req, gqlInfo) => root[nodeKey](args, req, gqlInfo);
    root[nodeKey] = async (args, req, gqlInfo) => {
        const nodeId = args.nodeId;
        const sp = nodeId.split(':');
        const r = await root[toCamelCase(sp[0])]({
            id: sp[1],
        }, req, gqlInfo);
        r.__typename = sp[0];
        return r;
    ...
    return {
        ...
        typeDefs: `${types
            ...
            const whereTypeSubFieldsStr = whereTypeSubFields.length > 0
...
function getGraphqlBaseType(field) {
    ...
    switch (field.valueType) {
        ...
        case Number:
            {
                if (field.valueConverter?.fieldTypeInDb === 'integer' ||
...
//@internal
export function translateWhereToRestBody(fields, { where }) {
    if (!where)
        return undefined;
    const result = {};
    for (const field of fields) {
        if (field.options.includeInApi === false)
            continue;
        const condition = where[field.key];
        if (condition) {
            const tr = (key, what) => {
