...
const HomeScreen = () => {
  const { trendingContent } = useGetTrendingContent() as { trendingContent: (Movie | TvShow) };
  ...
  return (
    <>
        ...
        <div className='absolute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32'>
					...
					<div className='max-w-2xl'>
						<h1 className='mt-4 text-6xl font-extrabold text-balance'>
							{(trendingContent as Movie)?.title || (trendingContent as TvShow)?.name}
						</h1>
      ...
      <div className='flex flex-col gap-10 bg-black py-10'>
				{contentType === "movie"
					? MOVIE_CATEGORIES.map((category) => <MovieSlider key={category} category={category} contType={"empty"} />)
					: TV_CATEGORIES.map((category) => <MovieSlider key={category} category={category} contType={"empty"} />)}
			</div>
    </>
  )
}
