...
import { LikedContent } from "../models/liked-content.js";
import Groq from "groq-sdk";

const groq = new Groq({ apiKey: ENV_VARS.GROQ_API_KEY });

export default class RecommenderController {
  static async AddedLikedContent(req, res) {
  ...
  static async RemoveLikedContent(req, res) {
    const { Content, userId } = req.body;
  ...
  static async getLikedContent(req, res) {
  ...
  static async getRecommendetions(req, res) {
    try {
      const userId = req.params.id;
      ...
      if (groqAIResponse) {
        ...
        const allRecommendedMap = new Map([
          ...recommendedMovies.map(
            (movie) => [movie.id, { ...movie, media_type: "movie" }]
          ),
          ...recommendedTvShows.map(
            (tvShow) => [tvShow.id, { ...tvShow, media_type: "tv" }]
          ),
        ]);
